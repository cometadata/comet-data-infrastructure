...
class TestCheckResumeKeys:
    def test_accepts_matching_state(self):
        state = {"next_batch": 3, "batch_size": 500, "sort_order": "chronological",
                 "shuffle_seed": None, "total_entries": 10000}
        current = {"batch_size": 500, "sort_order": "chronological",
                   "shuffle_seed": None, "total_entries": 10000}
        check_resume_keys(state, current, "some-paths")

    def test_rejects_mismatched_batch_size(self):
        state = {"next_batch": 3, "batch_size": 100, "sort_order": "chronological",
                 "shuffle_seed": None, "total_entries": 10000}
        current = {"batch_size": 500, "sort_order": "chronological",
                   "shuffle_seed": None, "total_entries": 10000}
...
class TestUploadBatchOutput:
    def test_invokes_s5cmd_cp_excluding_checkpoint(self, tmp_path, monkeypatch):
        calls = []
        monkeypatch.setattr(
            "comet.arxiv.pipeline.run_process", lambda args: calls.append(args)
        )
...
class TestResumeLoopArithmetic:
    ...
    @staticmethod
    def simulate(total: int, batch_size: int, start_batch: int,
                 max_batches: int | None) -> list[int]:
    ...
    def test_increased_max_batches_on_resume_matches_fresh_run(self):
        # First run did batches 0,1,2 with max_batches=3 → next_batch=3.
        # Resume with max_batches=5: should do 3,4 (total 5 == fresh --max-batches 5).
        assert self.simulate(total=10000, batch_size=500,
                             start_batch=3, max_batches=5) == [3, 4]

    def test_resume_without_max_batches_runs_to_end(self):
        assert self.simulate(total=2000, batch_size=500,
                             start_batch=2, max_batches=None) == [2, 3]

    def test_smaller_max_batches_on_resume_does_nothing(self):
        # First run did batches 0,1,2 with no cap → next_batch=3.
        # Resume with max_batches=2: slice keeps 2 batches; loop skips both.
        assert self.simulate(total=10000, batch_size=500,
                             start_batch=3, max_batches=2) == []

    def test_fresh_run_no_resume(self):
        assert self.simulate(total=1500, batch_size=500,
                             start_batch=0, max_batches=None) == [0, 1, 2]
