"""Tests for comet.xml2parquet."""

import textwrap
from datetime import datetime

import pyarrow as pa
import pyarrow.parquet as pq
import pytest

from comet.arxiv.manifest_parquet import ARXIV_MANIFEST_SCHEMA, convert_arxiv_manifest

SAMPLE_XML = textwrap.dedent("""\
    <?xml version='1.0' standalone='yes'?>
    <arXivSRC>
      <file>
        <content_md5sum>aaa111</content_md5sum>
        <filename>src/arXiv_src_0001_001.tar</filename>
        <first_item>astro-ph0001001</first_item>
        <last_item>quant-ph0001119</last_item>
        <md5sum>bbb222</md5sum>
        <num_items>2364</num_items>
        <seq_num>1</seq_num>
        <size>225605507</size>
        <timestamp>2010-12-23 00:13:59</timestamp>
        <yymm>0001</yymm>
      </file>
      <file>
        <content_md5sum>ccc333</content_md5sum>
        <filename>src/arXiv_src_0002_001.tar</filename>
        <first_item>astro-ph0002001</first_item>
        <last_item>quant-ph0002094</last_item>
        <md5sum>ddd444</md5sum>
        <num_items>2365</num_items>
        <seq_num>1</seq_num>
        <size>227036528</size>
        <timestamp>2010-12-23 00:18:09</timestamp>
        <yymm>0002</yymm>
      </file>
      <timestamp>Sat Mar  7 05:47:43 2026</timestamp>
    </arXivSRC>
""")

EMPTY_XML = textwrap.dedent("""\
    <?xml version='1.0' standalone='yes'?>
    <arXivPDF>
      <timestamp>Sat Mar  7 05:47:43 2026</timestamp>
    </arXivPDF>
""")


class TestConvertArxivManifest:
    def test_produces_correct_rows_and_schema(self, tmp_path):
        xml_file = tmp_path / "manifest.xml"
        xml_file.write_text(SAMPLE_XML)

        output_path = convert_arxiv_manifest(xml_file, tmp_path / "out")

        table = pq.read_table(output_path)
        assert table.schema.equals(ARXIV_MANIFEST_SCHEMA)
        assert table.num_rows == 2
        assert table.column("content_md5sum").to_pylist() == ["aaa111", "ccc333"]
        assert table.column("filename").to_pylist() == [
            "src/arXiv_src_0001_001.tar",
            "src/arXiv_src_0002_001.tar",
        ]

    def test_typed_fields_are_correct(self, tmp_path):
        xml_file = tmp_path / "manifest.xml"
        xml_file.write_text(SAMPLE_XML)

        output_path = convert_arxiv_manifest(xml_file, tmp_path / "out")

        table = pq.read_table(output_path)
        assert table.column("num_items").to_pylist() == [2364, 2365]
        assert table.column("seq_num").to_pylist() == [1, 1]
        assert table.column("size").to_pylist() == [225605507, 227036528]
        assert table.column("timestamp").to_pylist() == [
            datetime(2010, 12, 23, 0, 13, 59),
            datetime(2010, 12, 23, 0, 18, 9),
        ]

    def test_empty_manifest_produces_empty_parquet_with_schema(self, tmp_path):
        xml_file = tmp_path / "empty.xml"
        xml_file.write_text(EMPTY_XML)

        output_path = convert_arxiv_manifest(xml_file, tmp_path / "out")

        table = pq.read_table(output_path)
        assert table.schema.equals(ARXIV_MANIFEST_SCHEMA)
        assert table.num_rows == 0

    def test_output_filename_matches_input(self, tmp_path):
        xml_file = tmp_path / "arXiv_src_manifest.xml"
        xml_file.write_text(SAMPLE_XML)

        output_path = convert_arxiv_manifest(xml_file, tmp_path / "out")

        assert output_path.name == "arXiv_src_manifest.parquet"

    def test_missing_input_file_raises(self, tmp_path):
        with pytest.raises(FileNotFoundError):
            convert_arxiv_manifest(tmp_path / "nonexistent.xml", tmp_path / "out")
