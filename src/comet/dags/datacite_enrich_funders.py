...
from comet.constants import DATACITE_FUNDERS_ENRICHMENT, FULL_DIR
...
def create_datacite_enrich_funders_dag(dag_id: str, params: DataCiteEnrichFundersParams) -> DAG:
    ...
    def enrich_dag():
        ...
        checked = check_enrichment_unpublished(release, dataset=DATACITE_FUNDERS_ENRICHMENT.identifier)
        ror = fetch_ror_release(bucket_name=params.bucket_name)
        previous = resolve_previous_release(release, dataset=DATACITE_FUNDERS_ENRICHMENT.identifier)
        diff = diff_release(
        ...
        publish = publish_release_asset(asset=DATACITE_FUNDERS_ASSET, dataset=DATACITE_FUNDERS_ENRICHMENT.identifier)
