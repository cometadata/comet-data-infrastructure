...
# Images are tagged with sha- followed by the short commit sha. Override to push a throwaway tag
# when iterating on uncommitted changes (sha tags are immutable in ECR).
...
# Marple image (comet-<env>-marple): built from Dockerfile.marple, which fetches the
# marple commit pinned in versions.env. Requires the Marple changes —
# create-ror-index/index-ror entry points, /health endpoint, S3 ROR fetch — see
...
# Airflow image (comet-<env>-airflow): extends apache/airflow:slim with the comet
